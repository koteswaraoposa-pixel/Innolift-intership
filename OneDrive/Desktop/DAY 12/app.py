...
def create_table():
    ...
    existing_columns = {
        row["name"]
        for row in conn.execute("PRAGMA table_info(LoanPredictions)").fetchall()
    }
    expected_columns = {
        "risk_score": "REAL",
        "confidence": "REAL",
    ...
    for column_name, column_type in expected_columns.items():
        if column_name not in existing_columns:
            conn.execute(f"ALTER TABLE LoanPredictions ADD COLUMN {column_name} {column_type}")
    conn.commit()
    conn.close()
...
# Backward compatible: the app historically used a heuristic risk scorer.
# It is now deprecated in favor of the trained ML model.

def predict_default(form_data):
    # Keep function for reference, but do not use in main flow.
